...
subplot(2, 1, 1);
plot(t, sam, "LineWidth", 2);    % AM调制信号
hold on;
plot(t, A0 + mt, "r", "LineWidth", 2);
xlabel("t/时间"); ylabel("幅度"); title("AM调制信号及其包络");
axis([0, 0.02, -3.1, 3.1]);
...
subplot(2, 1, 2);
plot(mf, abs(msf), "Linewidth", 2);
xlabel("f/Hz"); ylabel("幅度/H(f)"); title("AM信号频域波形");
axis([-1500 1500 -inf inf]);
...
subplot(2, 2, 4);
y = abs(hilbert(sam)) - A0;
plot(t, y, "LineWidth", 2);
xlabel("t/时间"); ylabel("幅度"); title("包络检波信号");
axis([0 0.1 -1.5 1.5]);
line([0, 0.1], [0, 0], "color", "b", "Linewidth", 2);